...
export function TimezoneClock() {
  ...
  const [zones, setZones] = useState(() => {
    if (typeof window !== "undefined") {
      ...
      return stored ? JSON.parse(stored) : ["America/Los_Angeles", "America/New_York", "Europe/London"]
    }
  ...
  return (
    <div className="fixed top-6 right-6 z-50">
      <div
        className="glass-nav rounded-full px-4 py-2 shadow-lg shadow-black/5 dark:shadow-black/20 flex items-center gap-3 cursor-pointer hover:ring-2 hover:ring-blue-500"
      ...
      {isExpanded && (
        <div className="mt-4 p-4 w-[340px] sm:w-[400px] max-h-[80vh] overflow-y-auto bg-white/90 dark:bg-slate-800/90 backdrop-blur-md rounded-xl shadow-xl space-y-4">
          ...
          {zones.map((tz) => (
            <div key={tz} className={`flex justify-between items-center text-sm px-3 py-2 rounded-md transition-colors duration-300 ${isNight(tz) ? "bg-slate-100 dark:bg-slate-700" : "bg-yellow-50 dark:bg-yellow-900/20"}`}>
